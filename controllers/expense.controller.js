...
class ExpenseController {
  ...
  // Get expense statistics
  static async getEmployeeExpenseStats(req, res) {
    try {
      const { employee_id } = req.params;
      const stats = await ExpenseModel.getEmployeeExpenseStats(employee_id);
  ...
  static async approveExpense(req, res) {
    try {
      ...
      // Approve the expense
      const success = await ExpenseModel.updateExpenseStatus(
        id,
        "approved",
        userData,
        notes,
  ...
  static async downloadReceipt(req, res) {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${expense.receipt_original_name || "receipt" + fileExtension}"`,
      );
