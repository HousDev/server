...
class ExpenseModel {
  ...
  static async getAllExpenses(filters = {}) {
    try {
      ...
      let sql = `SELECT hex.*,concat(he.first_name,' ',he.last_name) as employee_name FROM hrms_expenses as hex LEFT JOIN hrms_employees he ON hex.employee_id = he.id ${whereClause} ORDER BY created_at DESC`;
  ...
  static async getExpenseById(id) {
    try {
      const rows = await query(`SELECT * FROM hrms_expenses WHERE id = ?`, [
        id,
      ]);
      return rows[0] || null;
    } catch (error) {
  ...
  static async deleteExpense(id) {
    try {
      const result = await query("DELETE FROM hrms_expenses WHERE id = ?", [
        id,
      ]);
      return result.affectedRows > 0;
    } catch (error) {
